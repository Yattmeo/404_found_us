from __future__ import annotations

import time
from datetime import datetime, timezone
from itertools import product
from typing import Any, Dict, List, Optional, Tuple

import numpy as np
import pandas as pd
from statsmodels.tsa.statespace.sarimax import SARIMAX

from config import (
    CALIBRATION_MAX_ABS_INTERCEPT,
    CALIBRATION_MAX_ABS_INTERCEPT_RELATIVE_TO_CONTEXT_MEAN,
    CALIBRATION_MAX_ABS_SLOPE,
    CALIBRATION_MIN_MATCHED_POINTS,
    CALIBRATION_MIN_PRED_STD,
    EXOGENOUS_FEATURE_COLUMNS,
    FALLBACK_MIN_HISTORY_WEEKS,
    SARIMA_D_FIXED,
    SARIMA_D_SEASONAL_FIXED,
    SARIMA_DEFAULT_P,
    SARIMA_DEFAULT_P_SEASONAL,
    SARIMA_DEFAULT_Q,
    SARIMA_DEFAULT_Q_SEASONAL,
    SARIMA_OPTIMISATION_TIMEOUT_S,
    SARIMA_P_CANDIDATES,
    SARIMA_P_SEASONAL_CANDIDATES,
    SARIMA_Q_CANDIDATES,
    SARIMA_Q_SEASONAL_CANDIDATES,
    SARIMA_SEASONAL_PERIOD,
)
from models import (
    CostForecastRequest,
    CostForecastResponse,
    ForecastWeek,
    ProcessMetadata,
    SarimaMetadata,
)


# ---------------------------------------------------------------------------
# Week helpers — must match the convention used by /getCompositeMerchant
# ---------------------------------------------------------------------------

def _week_of_year(date: pd.Timestamp) -> int:
    """
    52-week year convention: week_of_year = floor((day_of_year - 1) / 7) + 1,
    capped at 52.
    """
    return min((date.day_of_year - 1) // 7 + 1, 52)


# ---------------------------------------------------------------------------
# Onboarding aggregation
# ---------------------------------------------------------------------------

def _build_onboarding_weekly(txn_rows: List[Dict[str, Any]]) -> pd.DataFrame:
    """
    Aggregate raw onboarding transaction rows to weekly proc_cost_pct.

    - Accepts the date field under either 'date' or 'transaction_date'.
    - Rows missing 'amount', 'proc_cost', or with amount <= 0 are skipped.
    - Returns DataFrame[calendar_year, week_of_year, weekly_avg_txn_cost_pct].
      Empty DataFrame if no valid rows.
    """
    records = []
    for row in txn_rows:
        raw_date = row.get("transaction_date") or row.get("date")
        if raw_date is None:
            continue
        try:
            date = pd.to_datetime(raw_date)
        except Exception:
            continue

        amount = row.get("amount")
        proc_cost = row.get("proc_cost")
        if amount is None or proc_cost is None:
            continue
        try:
            amount = float(amount)
            proc_cost = float(proc_cost)
        except (TypeError, ValueError):
            continue
        if amount <= 0:
            continue

        records.append({
            "calendar_year": date.year,
            "week_of_year": _week_of_year(date),
            "txn_cost_pct": proc_cost / amount,
        })

    if not records:
        return pd.DataFrame(
            columns=["calendar_year", "week_of_year", "weekly_avg_txn_cost_pct"]
        )

    df = pd.DataFrame(records)
    return (
        df.groupby(["calendar_year", "week_of_year"], as_index=False)["txn_cost_pct"]
        .mean()
        .rename(columns={"txn_cost_pct": "weekly_avg_txn_cost_pct"})
    )


# ---------------------------------------------------------------------------
# SARIMA fitting helpers
# ---------------------------------------------------------------------------

def _fit_sarima(
    series: np.ndarray,
    order: Tuple[int, int, int],
    seasonal_order: Tuple[int, int, int, int],
    exog: Optional[np.ndarray] = None,
) -> Tuple[Any, str]:
    """
    Fit a SARIMAX model.  Returns (result, status_str).
    result is None on failure; status_str describes the outcome.
    """
    try:
        model = SARIMAX(
            series,
            exog=exog,
            order=order,
            seasonal_order=seasonal_order,
            enforce_stationarity=False,
            enforce_invertibility=False,
        )
        result = model.fit(disp=False)
        return result, "ok"
    except Exception as exc:  # noqa: BLE001
        return None, f"failed: {exc}"


def _grid_search_sarima(
    series: np.ndarray,
    timeout_s: float,
    exog: Optional[np.ndarray] = None,
) -> Tuple[Any, dict]:
    """
    AIC-guided grid search over SARIMA_*_CANDIDATES from config.
    Stops early if timeout_s wall-clock seconds are exceeded.

    Returns (best_result, metadata_dict).
    best_result is None if every candidate failed.
    """
    candidates = list(product(
        SARIMA_P_CANDIDATES, [SARIMA_D_FIXED], SARIMA_Q_CANDIDATES,
        SARIMA_P_SEASONAL_CANDIDATES, [SARIMA_D_SEASONAL_FIXED], SARIMA_Q_SEASONAL_CANDIDATES,
    ))

    best_result = None
    best_aic = np.inf
    best_order: Tuple[int, int, int] = (SARIMA_DEFAULT_P, SARIMA_D_FIXED, SARIMA_DEFAULT_Q)
    best_seasonal_order: Tuple[int, int, int, int] = (
        SARIMA_DEFAULT_P_SEASONAL, SARIMA_D_SEASONAL_FIXED,
        SARIMA_DEFAULT_Q_SEASONAL, SARIMA_SEASONAL_PERIOD,
    )
    evaluated = 0
    t0 = time.monotonic()

    for p, d, q, P, D, Q in candidates:
        if time.monotonic() - t0 > timeout_s:
            break
        result, _ = _fit_sarima(
            series,
            (p, d, q),
            (P, D, Q, SARIMA_SEASONAL_PERIOD),
            exog=exog,
        )
        evaluated += 1
        if result is not None and result.aic < best_aic:
            best_aic = result.aic
            best_result = result
            best_order = (p, d, q)
            best_seasonal_order = (P, D, Q, SARIMA_SEASONAL_PERIOD)

    elapsed_ms = (time.monotonic() - t0) * 1000
    return best_result, {
        "order": best_order,
        "seasonal_order": best_seasonal_order,
        "aic": float(best_aic) if best_result is not None else None,
        "evaluated": evaluated,
        "elapsed_ms": elapsed_ms,
    }


def _build_exog_matrices(
    sorted_features: List[Any],
    forecast_horizon_wks: int,
) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """
    Build historical and future exogenous matrices for SARIMAX.

    Future exogenous values are carried forward from the most recent observed
    composite week (last-value hold).
    """
    exog_hist = np.array(
        [
            [float(getattr(row, col, 0.0)) for col in EXOGENOUS_FEATURE_COLUMNS]
            for row in sorted_features
        ],
        dtype=float,
    )
    exog_hist = np.nan_to_num(exog_hist, nan=0.0, posinf=0.0, neginf=0.0)

    last_row = exog_hist[-1, :] if len(exog_hist) > 0 else np.zeros(len(EXOGENOUS_FEATURE_COLUMNS))
    exog_future = np.repeat(last_row[np.newaxis, :], forecast_horizon_wks, axis=0)

    return exog_hist, exog_future, list(EXOGENOUS_FEATURE_COLUMNS)


# ---------------------------------------------------------------------------
# Fallback response builder
# ---------------------------------------------------------------------------

def _build_fallback_response(
    req: CostForecastRequest,
    context_window_weeks: int,
    onboarding_mean: Optional[float],
    fallback_reason: str,
    generated_at: datetime,
) -> CostForecastResponse:
    """Return a flat constant forecast equal to onboarding_mean (or 0 if unavailable)."""
    fallback_value = onboarding_mean if onboarding_mean is not None else 0.0

    sarima_meta = SarimaMetadata(
        seasonal_length=SARIMA_SEASONAL_PERIOD,
        use_optimised_sarima=req.use_optimised_sarima,
        use_exogenous_sarimax=req.use_exogenous_sarimax,
        exogenous_feature_names=list(EXOGENOUS_FEATURE_COLUMNS) if req.use_exogenous_sarimax else [],
        selected_order=[0, 0, 0],
        selected_seasonal_order=[0, 0, 0, SARIMA_SEASONAL_PERIOD],
        aic=None,
        fit_status="fallback",
        optimisation_attempted=False,
        optimisation_time_ms=None,
        optimisation_candidates_evaluated=0,
    )

    proc_meta = ProcessMetadata(
        context_window_weeks_count=context_window_weeks,
        matched_calibration_points=0,
        calibration_mode="not_applicable",
        is_fallback=True,
        fallback_reason=fallback_reason,
        fallback_mean_proc_cost_pct=fallback_value,
        context_window_mean_proc_cost_pct=onboarding_mean,
        generated_at_utc=generated_at,
        forecast_horizon_wks=req.forecast_horizon_wks,
        confidence_interval=req.confidence_interval,
        used_guarded_calibration=req.use_guarded_calibration,
        calibration_successful=False,
        calibration_mae=None,
        calibration_rmse=None,
        calibration_r2=None,
    )

    forecast_weeks = [
        ForecastWeek(
            forecast_week_index=context_window_weeks + i + 1,
            proc_cost_pct_mid=fallback_value,
            proc_cost_pct_ci_lower=fallback_value,
            proc_cost_pct_ci_upper=fallback_value,
        )
        for i in range(req.forecast_horizon_wks)
    ]

    return CostForecastResponse(
        process_metadata=proc_meta,
        sarima_metadata=sarima_meta,
        is_guarded_sarima=False,
        forecast=forecast_weeks,
    )


# ---------------------------------------------------------------------------
# Main entry point
# ---------------------------------------------------------------------------

def get_cost_forecast(req: CostForecastRequest) -> CostForecastResponse:
    generated_at = datetime.now(timezone.utc)

    # -----------------------------------------------------------------------
    # 1. Sort composite history chronologically → positional index
    # -----------------------------------------------------------------------
    sorted_features = sorted(
        req.composite_weekly_features,
        key=lambda r: (r.calendar_year, r.week_of_year),
    )
    composite_series = np.array(
        [r.weekly_avg_txn_cost_pct_mean for r in sorted_features], dtype=float
    )
    exog_hist: Optional[np.ndarray] = None
    exog_future: Optional[np.ndarray] = None
    exogenous_feature_names: List[str] = []
    if req.use_exogenous_sarimax:
        exog_hist, exog_future, exogenous_feature_names = _build_exog_matrices(
            sorted_features,
            req.forecast_horizon_wks,
        )
    # Maps (calendar_year, week_of_year) → position in composite_series
    week_key_index: Dict[Tuple[int, int], int] = {
        (r.calendar_year, r.week_of_year): i for i, r in enumerate(sorted_features)
    }

    # -----------------------------------------------------------------------
    # 2. Build onboarding weekly series for calibration
    # -----------------------------------------------------------------------
    onboarding_weekly = _build_onboarding_weekly(req.onboarding_merchant_txn_df)
    if not onboarding_weekly.empty:
        onboarding_weekly = onboarding_weekly.sort_values(["calendar_year", "week_of_year"]).reset_index(drop=True)
    context_window_weeks = len(onboarding_weekly)
    onboarding_mean: Optional[float] = (
        float(onboarding_weekly["weekly_avg_txn_cost_pct"].mean())
        if not onboarding_weekly.empty
        else None
    )

    # -----------------------------------------------------------------------
    # 3. Guard: insufficient composite history → flat fallback
    # -----------------------------------------------------------------------
    if len(composite_series) < FALLBACK_MIN_HISTORY_WEEKS:
        return _build_fallback_response(
            req=req,
            context_window_weeks=context_window_weeks,
            onboarding_mean=onboarding_mean,
            fallback_reason=(
                f"Composite history has {len(composite_series)} weeks; "
                f"minimum required is {FALLBACK_MIN_HISTORY_WEEKS}."
            ),
            generated_at=generated_at,
        )

    # -----------------------------------------------------------------------
    # 4. Determine rolling-origin train/forecast split
    #    Train up to the week before onboarding starts, then generate through
    #    onboarding window + requested horizon.
    # -----------------------------------------------------------------------
    onboarding_positions: List[int] = []
    if not onboarding_weekly.empty:
        for _, row in onboarding_weekly.iterrows():
            key = (int(row["calendar_year"]), int(row["week_of_year"]))
            if key in week_key_index:
                onboarding_positions.append(week_key_index[key])

    if onboarding_positions and min(onboarding_positions) > 0:
        train_end_pos = min(onboarding_positions) - 1
        onboarding_end_pos = max(onboarding_positions)
    else:
        # Fallback to previous behavior anchor when onboarding keys are missing
        # or start at the very first composite point.
        train_end_pos = len(composite_series) - 1
        onboarding_end_pos = train_end_pos

    train_series = composite_series[: train_end_pos + 1]
    train_exog_hist: Optional[np.ndarray] = None
    if exog_hist is not None:
        train_exog_hist = exog_hist[: train_end_pos + 1, :]

    steps_to_onboarding_end = max(0, onboarding_end_pos - train_end_pos)
    total_generated_steps = steps_to_onboarding_end + req.forecast_horizon_wks

    # Build future exogenous values for the full generated path.
    exog_future_generated: Optional[np.ndarray] = None
    if train_exog_hist is not None:
        last_row = train_exog_hist[-1, :] if len(train_exog_hist) > 0 else np.zeros(len(EXOGENOUS_FEATURE_COLUMNS))
        exog_future_generated = np.repeat(last_row[np.newaxis, :], total_generated_steps, axis=0)

    # -----------------------------------------------------------------------
    # 5. Fit SARIMA on truncated history
    # -----------------------------------------------------------------------
    _default_order: Tuple[int, int, int] = (SARIMA_DEFAULT_P, SARIMA_D_FIXED, SARIMA_DEFAULT_Q)
    _default_seasonal: Tuple[int, int, int, int] = (
        SARIMA_DEFAULT_P_SEASONAL, SARIMA_D_SEASONAL_FIXED,
        SARIMA_DEFAULT_Q_SEASONAL, SARIMA_SEASONAL_PERIOD,
    )
    opt_meta: dict = {
        "order": _default_order,
        "seasonal_order": _default_seasonal,
        "aic": None,
        "evaluated": 0,
        "elapsed_ms": 0.0,
    }

    if req.use_optimised_sarima:
        sarima_result, opt_meta = _grid_search_sarima(
            train_series,
            SARIMA_OPTIMISATION_TIMEOUT_S,
            exog=train_exog_hist,
        )
        order: Tuple[int, int, int] = opt_meta["order"]
        seasonal_order: Tuple[int, int, int, int] = opt_meta["seasonal_order"]
        fit_status = "ok" if sarima_result is not None else "failed"
    else:
        order = _default_order
        seasonal_order = _default_seasonal
        sarima_result, fit_status = _fit_sarima(
            train_series,
            order,
            seasonal_order,
            exog=train_exog_hist,
        )

    if sarima_result is None:
        return _build_fallback_response(
            req=req,
            context_window_weeks=context_window_weeks,
            onboarding_mean=onboarding_mean,
            fallback_reason=f"SARIMA fit failed: {fit_status}",
            generated_at=generated_at,
        )

    # -----------------------------------------------------------------------
    # 6. Generate path from onboarding start through forecast horizon
    # -----------------------------------------------------------------------
    alpha = 1.0 - req.confidence_interval
    if exog_future_generated is not None:
        forecast_obj = sarima_result.get_forecast(steps=total_generated_steps, exog=exog_future_generated)
    else:
        forecast_obj = sarima_result.get_forecast(steps=total_generated_steps)
    # predicted_mean and conf_int may be a pandas Series/DataFrame or a plain
    # numpy array depending on statsmodels version; np.asarray normalises both.
    generated_mean: np.ndarray = np.asarray(forecast_obj.predicted_mean).copy()
    ci_arr = np.asarray(forecast_obj.conf_int(alpha=alpha))
    generated_lower: np.ndarray = ci_arr[:, 0].copy()
    generated_upper: np.ndarray = ci_arr[:, 1].copy()

    final_start_step = steps_to_onboarding_end
    final_end_step = final_start_step + req.forecast_horizon_wks
    forecast_mean: np.ndarray = generated_mean[final_start_step:final_end_step].copy()
    forecast_lower: np.ndarray = generated_lower[final_start_step:final_end_step].copy()
    forecast_upper: np.ndarray = generated_upper[final_start_step:final_end_step].copy()

    # -----------------------------------------------------------------------
    # 7. Calibration alignment — use generated onboarding segment
    # -----------------------------------------------------------------------

    calib_sarima: List[float] = []
    calib_actual: List[float] = []
    context_fitted_by_pos: List[Optional[float]] = [None] * context_window_weeks

    if not onboarding_weekly.empty:
        for row_idx, (_, row) in enumerate(onboarding_weekly.iterrows()):
            key = (int(row["calendar_year"]), int(row["week_of_year"]))
            if key not in week_key_index:
                continue
            pos = week_key_index[key]
            gen_step = pos - (train_end_pos + 1)
            if gen_step < 0 or gen_step >= len(generated_mean):
                continue
            pred_val = generated_mean[gen_step]
            if np.isnan(pred_val):
                continue
            context_fitted_by_pos[row_idx] = float(pred_val)
            calib_sarima.append(float(pred_val))
            calib_actual.append(float(row["weekly_avg_txn_cost_pct"]))

    matched_calibration_points = len(calib_sarima)

    # -----------------------------------------------------------------------
    # 8. Guarded linear calibration
    #    Midpoint: calibrated_mid = slope * sarima_mid + intercept
    #    CI bounds: shifted by the same offset so they remain centred on
    #               calibrated_mid and preserve SARIMA's original interval width.
    # -----------------------------------------------------------------------
    calibration_mode = "skipped_disabled"
    is_guarded_sarima = False
    calibration_successful = False
    calibration_mae: Optional[float] = None
    calibration_rmse: Optional[float] = None
    calibration_r2: Optional[float] = None

    if req.use_guarded_calibration:
        if matched_calibration_points < CALIBRATION_MIN_MATCHED_POINTS:
            calibration_mode = "skipped_insufficient_data"
        else:
            pred_arr = np.array(calib_sarima)
            actual_arr = np.array(calib_actual)
            pred_std = float(pred_arr.std())

            if pred_std < CALIBRATION_MIN_PRED_STD:
                calibration_mode = "skipped_flat_predictions"
            else:
                X = np.column_stack([pred_arr, np.ones(len(pred_arr))])
                try:
                    coeffs, _, _, _ = np.linalg.lstsq(X, actual_arr, rcond=None)
                    context_mean_abs = float(abs(actual_arr.mean())) if len(actual_arr) > 0 else 0.0
                    intercept_cap = max(
                        CALIBRATION_MAX_ABS_INTERCEPT,
                        CALIBRATION_MAX_ABS_INTERCEPT_RELATIVE_TO_CONTEXT_MEAN * context_mean_abs,
                    )
                    slope = float(
                        np.clip(coeffs[0], -CALIBRATION_MAX_ABS_SLOPE, CALIBRATION_MAX_ABS_SLOPE)
                    )
                    intercept = float(
                        np.clip(
                            coeffs[1],
                            -intercept_cap,
                            intercept_cap,
                        )
                    )

                    # Apply calibration only if it improves context-window fit.
                    raw_residuals = actual_arr - pred_arr
                    raw_rmse = float(np.sqrt(np.mean(np.square(raw_residuals))))
                    calibrated_arr = slope * pred_arr + intercept
                    residuals = actual_arr - calibrated_arr
                    calibrated_rmse = float(np.sqrt(np.mean(np.square(residuals))))
                    if calibrated_rmse >= raw_rmse:
                        calibration_mode = "skipped_no_improvement"
                    else:
                        # Calibrate midpoint; shift CI bounds by the same offset so
                        # they stay centred on the calibrated midpoint.
                        original_mean = forecast_mean.copy()
                        forecast_mean = slope * forecast_mean + intercept
                        offset = forecast_mean - original_mean
                        forecast_lower = forecast_lower + offset
                        forecast_upper = forecast_upper + offset

                        calibration_mode = "guarded_linear"
                        is_guarded_sarima = True
                        calibration_successful = True

                        calibration_mae = float(np.mean(np.abs(residuals)))
                        calibration_rmse = calibrated_rmse
                        ss_res = float(np.sum(np.square(residuals)))
                        ss_tot = float(np.sum(np.square(actual_arr - actual_arr.mean())))
                        calibration_r2 = float(1.0 - ss_res / ss_tot) if ss_tot > 0 else None
                except np.linalg.LinAlgError:
                    calibration_mode = "skipped_linalg_error"

    # -----------------------------------------------------------------------
    # 9. Assemble response
    # -----------------------------------------------------------------------
    aic_value: Optional[float] = (
        opt_meta["aic"]
        if req.use_optimised_sarima
        else (float(sarima_result.aic) if sarima_result is not None else None)
    )

    sarima_meta = SarimaMetadata(
        seasonal_length=SARIMA_SEASONAL_PERIOD,
        use_optimised_sarima=req.use_optimised_sarima,
        use_exogenous_sarimax=req.use_exogenous_sarimax,
        exogenous_feature_names=exogenous_feature_names,
        selected_order=list(order),
        selected_seasonal_order=list(seasonal_order),
        aic=aic_value,
        fit_status=fit_status,
        optimisation_attempted=req.use_optimised_sarima,
        optimisation_time_ms=opt_meta["elapsed_ms"] if req.use_optimised_sarima else None,
        optimisation_candidates_evaluated=opt_meta["evaluated"] if req.use_optimised_sarima else 0,
    )

    proc_meta = ProcessMetadata(
        context_window_weeks_count=context_window_weeks,
        matched_calibration_points=matched_calibration_points,
        calibration_mode=calibration_mode,
        is_fallback=False,
        fallback_reason=None,
        fallback_mean_proc_cost_pct=None,
        context_window_mean_proc_cost_pct=onboarding_mean,
        generated_at_utc=generated_at,
        forecast_horizon_wks=req.forecast_horizon_wks,
        confidence_interval=req.confidence_interval,
        used_guarded_calibration=req.use_guarded_calibration,
        calibration_successful=calibration_successful,
        calibration_mae=calibration_mae,
        calibration_rmse=calibration_rmse,
        calibration_r2=calibration_r2,
    )

    if onboarding_weekly.empty:
        output_start_week_index = context_window_weeks + 1
    else:
        output_start_week_index = int(onboarding_weekly["week_of_year"].min()) + context_window_weeks

    forecast_weeks = []
    for i in range(req.forecast_horizon_wks):
        forecast_weeks.append(
            ForecastWeek(
                forecast_week_index=output_start_week_index + i,
                proc_cost_pct_mid=float(forecast_mean[i]),
                proc_cost_pct_ci_lower=float(forecast_lower[i]),
                proc_cost_pct_ci_upper=float(forecast_upper[i]),
            )
        )

    return CostForecastResponse(
        process_metadata=proc_meta,
        sarima_metadata=sarima_meta,
        is_guarded_sarima=is_guarded_sarima,
        forecast=forecast_weeks,
        context_sarima_fitted=context_fitted_by_pos,
    )
