...
Real-data note:
- In `processed_transactions_4mcc.csv`, `card_type` values are typically
  `debit`, `credit`, and `debit (prepaid)`.
- Requests should pass these literal values in `card_types` when evaluating on
  the provided real dataset.
...
Integration note:
- `/getCompositeMerchant` output is consumed directly by the GetCostForecast
  service (`/GetCostForecast`) via the `weekly_features` array.
