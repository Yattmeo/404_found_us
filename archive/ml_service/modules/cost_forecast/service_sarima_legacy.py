...
from .models import CostForecastRequest, CostForecastResponse, ForecastWeek, ProcessMetadata, SarimaMetadata
...
def _build_onboarding_weekly(txn_rows: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    if not records:
        return pd.DataFrame(columns=["calendar_year", "week_of_year", "weekly_avg_txn_cost_pct"])
...
def _grid_search_sarima(series: np.ndarray, timeout_s: float, exog: Optional[np.ndarray] = None) -> Tuple[Any, dict]:
    candidates = list(
        product(
            SARIMA_P_CANDIDATES,
            [SARIMA_D_FIXED],
...
def _build_exog_matrices(sorted_features: List[Any], forecast_horizon_wks: int) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    exog_hist = np.array(
...
def get_cost_forecast(req: CostForecastRequest) -> CostForecastResponse:
    ...
    composite_series = np.array([r.weekly_avg_txn_cost_pct_mean for r in sorted_features], dtype=float)

    exog_hist: Optional[np.ndarray] = None
    exogenous_feature_names: List[str] = []
    if req.use_exogenous_sarimax:
        exog_hist, _, exogenous_feature_names = _build_exog_matrices(sorted_features, req.forecast_horizon_wks)
    ...
    onboarding_mean: Optional[float] = (
        float(onboarding_weekly["weekly_avg_txn_cost_pct"].mean()) if not onboarding_weekly.empty else None
    )
    ...
    if req.use_optimised_sarima:
        sarima_result, opt_meta = _grid_search_sarima(train_series, SARIMA_OPTIMISATION_TIMEOUT_S, exog=train_exog_hist)
