...
@dataclass
class KNNRateQuoteResult:
    forecast_proc_cost: List[float]
    context_len: int
    horizon_len: int
    k: int
    end_month: int
...
class KNNRateQuoteService:
    def __init__(
        ...
        if base_dir is None:
            base_dir = Path(__file__).parent / "Clustering" / "Dataset Creation"
        self.base_dir = base_dir
        self.db_path = Path(__file__).parent / "rate_quote.sqlite"

        self._ensure_sqlite_loaded()
    ...
    def _load_cost_type_ids(self) -> List[str]:
        with sqlite3.connect(self.db_path) as conn:
            cost_type_ref = pd.read_sql(
                "SELECT cost_type_ID FROM cost_type_ref", conn
            )
        return (
    ...
    def _load_monthly_reference(self) -> pd.DataFrame:
        with sqlite3.connect(self.db_path) as conn:
            all_df = pd.read_sql("SELECT * FROM transactions", conn)
    ...
    def _build_monthly_features(self, df: pd.DataFrame, card_type: Optional[str] = None) -> pd.DataFrame:
        df = df.copy()
    ...
    def _build_pool_by_month(self, monthly_data: pd.DataFrame, card_type: str = "both") -> Tuple[Dict[int, pd.DataFrame], Dict[int, pd.Series]]:
        """Build pool from filtered monthly data for a specific card type."""
        ...
        for end_period in all_periods:
            ...
            ctx_df = monthly_data[
                monthly_data["ym_period"].isin(context_periods)
            ]
            target_df = monthly_data[
                monthly_data["ym_period"].isin(target_periods)
            ]
    ...
    def _ensure_sqlite_loaded(self) -> None:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='cost_type_ref'"
            )
            cost_ref_exists = cursor.fetchone() is not None

            if transactions_exists and cost_ref_exists:
                return

            splits_dir = self.base_dir / "5411_temporal_splits"
            train_path = splits_dir / "5411_train_temporal_with_costs.csv"
            validate_path = splits_dir / "5411_validate_temporal_with_costs.csv"
            test_path = splits_dir / "5411_test_temporal_with_costs.csv"

            train_df = pd.read_csv(train_path)
            validate_df = pd.read_csv(validate_path)
            test_df = pd.read_csv(test_path)
            all_df = pd.concat([train_df, validate_df, test_df], ignore_index=True)
            all_df.to_sql("transactions", conn, if_exists="replace", index=False)

            cost_type_path = self.base_dir / "cost_type_id_18feb.csv"
    ...
    def _get_pool(self, month_num: int, card_type: str = "both") -> Optional[pd.DataFrame]:
        ...
        # Build pool from filtered monthly data
        pools, _ = self._build_pool_by_month(filtered_monthly, card_type)
    ...
    def _get_pool_means(self, month_num: int, card_type: str = "both") -> Optional[pd.Series]:
        """Get feature means for a card-type-specific pool."""
        cache_key = (month_num, card_type)
        ...
        # Build pool from filtered monthly data
        _, pool_means = self._build_pool_by_month(filtered_monthly, card_type)
    ...
    def _compute_query_features(
        ...
        if df is None:
            ...
            # If as_of_date is beyond the data, use same month from most recent year
...
__all__ = ["KNNRateQuoteService", "KNNRateQuoteResult"]
