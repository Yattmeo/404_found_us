...
OUTPUT_DIR = ROOT / "ml_pipeline" / "Matt_EDA" / "service_eval_outputs" / "multi" / "latest"
DB_PATH = OUTPUT_DIR / "multi_merchant_eval.sqlite"
METRICS_PATH = OUTPUT_DIR / "multi_merchant_eval_metrics.json"
SWEEP_METRICS_PATH = OUTPUT_DIR / "multi_merchant_eval_sweep_metrics.json"
PLOT_PATH = OUTPUT_DIR / "multi_merchant_forecast_panels.png"
...
def _select_merchants(
    ...
    for merchant_id, count in counts_eval_year.items():
        ...
        # Need Jan data + context weeks + horizon weeks
...
def _plot_all_merchants(eval_rows: List[Dict[str, Any]], output_path: Path) -> None:
    ...
    for i, row in enumerate(eval_rows):
        ...
        # Target actuals across context + forecast window.
        ...
        # Raw SARIMA on context + forecast.
        ...
        # Context mean and forecast boundary.
    ...
    fig.suptitle("Multi-merchant evaluation: Context + Forecast (Actual vs Raw vs Calibrated)", fontsize=14)
    fig.tight_layout()
    ...
    try:
        ...
        for merchant_id in target_merchants:
            ...
            onboarding_quote_df = target_full_df[
                target_full_df["date_dt"].dt.to_period("M") == pd.Period(f"{eval_year}-01", freq="M")
            ].copy()
            onboarding_weekly_df = target_full_df[
                target_full_df["week_of_year"].between(1, context_weeks)
            ].copy()
            ...
            cost_forecast_raw_resp = _post_json(f"http://127.0.0.1:{FORECAST_PORT}/GetCostForecast", forecast_req_raw)

            pred_mid = np.array([w["proc_cost_pct_mid"] for w in cost_forecast_resp["forecast"]], dtype=float)
            pred_mid_raw = np.array([w["proc_cost_pct_mid"] for w in cost_forecast_raw_resp["forecast"]], dtype=float)
            ...
            raw_context = np.array(
                [float("nan") if v is None else float(v) for v in cost_forecast_raw_resp.get("context_sarima_fitted", [])],
                dtype=float,
            )
            ...
            rec = {
                ...
                "sarima": cost_forecast_resp.get("sarima_metadata", {}),
            }
            per_merchant.append(rec)

            context_mean = cost_forecast_resp.get("process_metadata", {}).get("context_window_mean_proc_cost_pct")
            ...
            print(
                ...
                f"KNN MAE={knn_mae:.4f}, Forecast MAE={forecast_mae:.4f}, "
        ...
        knn_maes = np.array([m["knn_mae"] for m in per_merchant], dtype=float)
        knn_rmses = np.array([m["knn_rmse"] for m in per_merchant], dtype=float)
        ...
        summary = {
            ...
            "knn": {
                "mae_mean": float(knn_maes.mean()),
                "mae_median": float(np.median(knn_maes)),
                "mae_std": float(knn_maes.std()),
                "rmse_mean": float(knn_rmses.mean()),
                "rmse_median": float(np.median(knn_rmses)),
                "rmse_std": float(knn_rmses.std()),
            },
            "get_cost_forecast": {
            ...
            "get_cost_forecast_corrected_observed_only": {
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run multi-merchant service evaluation.")
