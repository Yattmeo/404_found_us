...
def build_monthly_features(df: pd.DataFrame, cost_type_ids: List[str]) -> pd.DataFrame:
    ...
    counts = (
        tx.groupby(["merchant_id", "ym", "cost_type_ID"])  # type: ignore[arg-type]
        .size()
        .rename("count")
        .reset_index()
    )
    ...
    features = pd.concat(
        [cost_pct, total_txn, avg_amount, proc_cost_pct], axis=1
    ).reset_index()
...
def build_pool_by_month(
    ...
    for end_period in all_periods:
        ...
        # Context feature vector per merchant (averaged over context window)
        ctx_agg = ctx.groupby("merchant_id")[feature_cols].mean()

        # Only keep merchants that have rows in all horizon months so the
        # forecast lookup is guaranteed to succeed. Values are NOT stored here.
    ...
    return {
        m: pd.concat(frames, axis=0, ignore_index=True)
        for m, frames in pool_by_end_month.items()
    }
...
def query_vector_from_txn_df(
    ...
    in_month["cost_type_ID"] = in_month.get("cost_type_ID").fillna(-1).astype(int).astype(str)
...
def lookup_horizon_proc_cost_pct(
    ...
    """For each matched pool case, look up the actual proc_cost_pct
    for the next horizon_len_months months from historical data."""
    results: List[List[float]] = []
    for _, row in matched_cases.iterrows():
        mid = row["merchant_id"]
        end_p = pd.Period(str(row["end_period"]), freq="M")
        month_values: List[float] = []
        for i in range(1, horizon_len_months + 1):
            target_p = end_p + i
            match = monthly_df[
                (monthly_df["merchant_id"] == mid)
                & (monthly_df["ym_period"] == target_p)
            ]
