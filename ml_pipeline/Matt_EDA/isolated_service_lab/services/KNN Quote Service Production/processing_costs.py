...
class HeuristicProcessingCostProvider:
    """Fallback estimator used when no external service is configured."""
...
class ExternalProcessingCostProvider:
    """Calls an external processing-cost service and expects a proc_cost column back."""

    endpoint: str
    timeout_seconds: int = 10
    fallback_provider: ProcessingCostProvider = field(
        default_factory=HeuristicProcessingCostProvider
    )
