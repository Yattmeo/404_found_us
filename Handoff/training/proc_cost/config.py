# ---------------------------------------------------------------------------
# GetCostForecast (proc_cost) Service — Configuration Constants
#
...
# ---------------------------------------------------------------------------
# Artifact storage
# Artifacts are written to ml_service/artifacts/proc_cost/{mcc}/{ctx_len}/
# and hot-reloaded by the running ml-service container.
# Override via env-var PROC_COST_ARTIFACTS_BASE_PATH if needed.
# ---------------------------------------------------------------------------
ARTIFACTS_BASE_PATH: Path = Path(
    os.getenv(
        "PROC_COST_ARTIFACTS_BASE_PATH",
        str(Path(__file__).parent.parent.parent / "ml_service" / "artifacts" / "proc_cost"),
    )
)
...
# When a change in training artifacts is detected, the new model is loaded and replaces the old one.
# ---------------------------------------------------------------------------
