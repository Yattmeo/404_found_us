"""
train.py — Monthly batch retraining script for GetCostForecast (proc_cost v2).
...
For each context length the script:
  ...
  10. Writes all artifacts atomically to ml_service/artifacts/proc_cost/{mcc}/{ctx_len}/.
...
def _train_context_len(
    ...
    # ── 3. Build features & fit proc_cost models ─────────────────────────────────
...
def train(mcc: int, data_path: Path, window_years: int) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"proc_cost v2 Training  MCC={mcc}  window={window_years}yr  data={data_path.name}")
    print(f"{'='*60}")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train proc_cost v2 model artifacts for GetCostForecast service.",
