...
suitable for training:
  - training/proc_cost/train.py  (--mcc <N> --data-path ...)
  - training/tpv/train.py        (--mcc <N> --data-path ...)

Input columns expected (e.g. training/data/processed_transactions_4mcc.csv):
  transaction_id, merchant_id, date, amount, mcc, card_brand,
  card_type, cost_type_ID, proc_cost, year, week

Output per MCC  (e.g. training/data/5411_monthly_v2.csv):
...
Usage:
  ...
  python prepare_data.py --input training/data/processed_transactions_4mcc.csv --output-dir training/data
...
# Constants
# ---------------------------------------------------------------------------
ROOT = Path(__file__).parent                         # Handoff/training/
DEFAULT_INPUT = ROOT / "data" / "processed_transactions_4mcc.csv"
DEFAULT_OUTPUT_DIR = ROOT / "data"
COST_TYPE_REFERENCE_CSV = ROOT.parent / "cost_structure" / "cost_type_id.csv"


def _load_cost_type_ids() -> list[int]:
    """Read cost_type_ID values from the reference CSV.

    The reference CSV (cost_structure/cost_type_id.csv) is the source of truth
    for the set of valid cost type IDs.  When the JSON fee schedules change and
    the CSV is regenerated, this function automatically picks up the new IDs
    without requiring a code change.
    """
    if not COST_TYPE_REFERENCE_CSV.exists():
        raise FileNotFoundError(
            f"Cost type reference CSV not found: {COST_TYPE_REFERENCE_CSV}\n"
            "Generate it from cost_structure/*.JSON per DATA_SPEC.md Section 2."
        )
    ref = pd.read_csv(COST_TYPE_REFERENCE_CSV)
    ids = sorted(ref["cost_type_ID"].dropna().astype(int).unique().tolist())
    if not ids:
        raise ValueError(f"No cost_type_ID values found in {COST_TYPE_REFERENCE_CSV}")
    return ids


COST_TYPE_IDS: list[int] = _load_cost_type_ids()
...
def _aggregate_to_monthly(txn_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (mid, yr, mo), g in df.groupby(["merchant_id", "year", "month"]):
        ...
        row: dict = {
            ...
            "year": int(yr),
            "month": int(mo),
            # proc_cost target + v2 cost-pct features
            ...
            # TPV target + v2 amount features (shared with proc_cost)
...
def main() -> None:
    ...
    for mcc in mcc_list:
        ...
        print(f"\n  # MCC {mcc} \u2014 proc_cost forecast:")
        print(f"  cd proc_cost")
