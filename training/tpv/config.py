...
# ---------------------------------------------------------------------------
# Artifact storage
# Artifacts are written to ml_service/artifacts/tpv/{mcc}/{ctx_len}/
# and hot-reloaded by the running ml-service container.
# Override via env-var TPV_ARTIFACTS_BASE_PATH if needed.
# ---------------------------------------------------------------------------
ARTIFACTS_BASE_PATH: Path = Path(
    os.getenv(
        "TPV_ARTIFACTS_BASE_PATH",
        str(Path(__file__).parent.parent.parent / "ml_service" / "artifacts" / "tpv"),
    )
)
